...
* **CI/CD:** GitHub Actions optimized with **`uv`** caching. Scrapers run inside **Official Playwright Docker Containers** (e.g., `v1.57.0-noble`) to eliminate system dependency installation time (`install-deps`), ensuring consistent environments and faster builds.
...
1.  **Entry Point:**
    ...
    * `pyproject.toml`: Project configuration and dependency definition (pinned `playwright` version to match CI container).
...
5.  **Deployment Workflows (`.github/workflows/`):**
    * `scrape.yml`: Hourly job. Full scraping pipeline using `uv` inside a Playwright Container.
    * `scrape-fast.yml`: Manual trigger. Containerized fast scrape (no Facebook).
    * `refresh_code.yml`: Manual trigger to regenerate site/assets without scraping (runs on standard runner with `uv`).
...
commitMessage: "docs: update prompt.md to reflect uv migration and containerized CI"
}